...
public class FragmentTabHost extends TabHost
        ...
        if (mLastTab != newTab) {
            ...
            if (mLastTab != null) {
                if (mLastTab.fragment != null) {
                    ft.hide(mLastTab.fragment); //将原有detach、attach改为hide，show
                }
            }
    ...
    public void setCurrentTab(int index) {
        if (mOnBeforeTabChangeListener != null && mOnBeforeTabChangeListener.beforeChanged(index)) {
            return;
        }
        super.setCurrentTab(index);
    }

    private OnBeforeTabChangeListener mOnBeforeTabChangeListener;

    public void setOnBeforeTabChangeListener(OnBeforeTabChangeListener onBeforeTabChangeListener) {
        this.mOnBeforeTabChangeListener = onBeforeTabChangeListener;
    }

    public void setTabArgs(String tabId, Bundle args) {
        TabInfo tabInfo = getTabInfoForTag(tabId);

        if (tabInfo == null) {
            return;
        }
        int index = mTabs.indexOf(tabInfo);
        mTabs.remove(index);
        mTabs.add(index, new TabInfo(tabInfo.tag, tabInfo.clss, args));
    }

    public static interface OnBeforeTabChangeListener {
        boolean beforeChanged(int index);
    }
}
