...
public class KeyboardWatcher {
    ...
    public KeyboardWatcher(Dialog dialog) {
    ...
    private void initialize() {
        if (hasAdjustResizeInputMode()) {
            ...
            rootViewRef = new WeakReference<>(dialogRef.get().findViewById(Window.ID_ANDROID_CONTENT));
            ...
            throw new IllegalArgumentException(String.format("Activity %s should have windowSoftInputMode=\"adjustResize\"" +
                    "to make KeyboardWatcher working. You can set it in AndroidManifest.xml", dialogRef.get().getClass().getSimpleName()));
        }
    }

    private boolean hasAdjustResizeInputMode() {
        return (dialogRef.get().getWindow().getAttributes().softInputMode & WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE) != 0;
    }
